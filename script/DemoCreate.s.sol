...
///         price, fund the vault from the deployer wallet, and buy a protection note
///         on the registered asset. Premium and liability come out of the same wallet,
///         so no second account is needed.
///
...
///                        DEMO_DURATION (seconds, default 1 days; the priced terms are
///                        1/7/14/30 days — anything else reverts InvalidDuration)
///
...
contract DemoCreate is ScriptBase {
    function run() external {
        ...
        uint256 duration = vmEnvUint("DEMO_DURATION", 1 days);
        ...
        (uint256 premiumUSD18,,) = note.quote(asset, amount, level, duration);
        ...
        vault.deposit(fund);
        uint256 noteId = note.create(asset, amount, level, duration);
        vmStopBroadcast();
