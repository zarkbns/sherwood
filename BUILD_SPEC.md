...
Four bounds close the gaps an audit found around this basis:
...
- **Per-asset concentration cap** — `create` refuses a note if the asset's total active liability across all owners (`assetExposure[asset]`, decremented at settlement) plus this note's liability would exceed `maxAssetExposureBps` of the vault's current deposits (fixed at deploy, immutable — 30% on the live deployment). One stock's crash can only ever eat its capped slice of the vault; the cap is measured in reserved-liability units and checked before any token can move.
...
- **Vault reserve buffer:** 2000 bps (20%) — configurable by owner, cap 5000 bps, never withdrawable by anyone while set (§4 `withdraw`/`redeem`), and a raise that would strand existing reserves is refused (§4 `setBufferBps`)
...
User ──> ProtectionNote (struct registry, noteId-keyed)
          ...
          │           read entry price → concentration cap check → consume noteId →
          │           vault.reserveFor (capacity check → split premium → collect) → record
...
### SherwoodVault.sol
Holds settlement token. Tracks `totalDeposits` (backer deposits + the backer share of received premiums), `reserved` (Σ active note liabilities, token units), and the backer share ledger `totalShares`/`sharesOf`. Sherwood's cut of each premium accrues to `pendingProtocolFees` — a bucket that never counts toward `totalDeposits`: it backs nothing, so claiming it can never strand a reserve, and the physical-balance invariant `balanceOf(this) ≥ totalDeposits + pendingProtocolFees` holds through every path (deposit, reserveFor, settlePayout, sweep).
...
- `deposit(amount)` — pull token, `totalDeposits += amount`, mint `amount × totalShares / totalDeposits` shares to the depositor (1:1 into an empty vault; a dust deposit that would mint zero shares is refused rather than accepted as a donation). Premiums collected via `reserveFor` raise `totalDeposits` without minting shares, so earlier backers' shares earn the premiums, and payouts shrink `totalDeposits` so the same shares bear the losses.
- `reserveFor(noteId, payer, premium, liability)` — **capacity check first**: `premium + liability ≤ availableCapacity()`. That is the plain `reserved + liability ≤ totalDeposits − buffer` precondition tightened by the premium itself, and it implies invariant 1 with room to spare. Then the premium splits — `protocolCut = premium × protocolFeeBps / 10_000` into `pendingProtocolFees`, the backer remainder into `totalDeposits` — and `reserved += liability`, and **only then** the premium is pulled from the payer (checks-effects-interactions). The capacity check runs on pre-premium deposits, so the fee slice can only ever make backing tighter, never looser. Called only by ProtectionNote.
...
- `withdraw(assets)` / `redeem(shares)` — backer exits, capped at `availableCapacity()`, so they can take neither reserved collateral nor the reserve buffer. The cap is exactly what keeps invariant 1 true *after* a withdrawal (`assets ≤ D(1−b) − R ⟹ (D − assets)(1−b) ≥ R`). `withdraw` burns shares at `ceilDiv` (rounds up against the caller) and `redeem` pays out at floor — the pool keeps the dust, never the reverse. Winding a vault fully down is therefore a deliberate two-step for depositors: settle the notes, `setBufferBps(0)`, withdraw.
- `availableCapacity()` view — `min(totalDeposits − buffer − reserved)` clamped at 0
- Owner: `setBufferBps` (a raise is refused with `BufferBreachesReserves` unless `reserved ≤ totalDeposits × (1 − newBuffer)` still holds — otherwise an admin call would strand collateral that was legitimately reserved under a smaller buffer; lowerings always pass), `setNoteContract` (**set-once**: the first binding after deploy is final — repointing mid-life would strand every active note's reserve behind the old contract's `onlyNote` gate, and upgrades are full-stack redeploys, matching the immutable settlement token), `setFeeTerms(newProtocolFeeBps, newTreasury)` (fee capped at 2500 bps), `claimProtocolFees()` — sweeps `pendingProtocolFees` to the treasury; the bucket never counted toward `totalDeposits`, so the sweep cannot touch the funds backing active notes.
...
- `calculatePayout(noteId, settlementPrice8)` — capped by the owner's current holding and by the claim window (zero past it), on the same basis as `settle()`, so the view can never promise more than settlement will pay.
...
`usdValue`, `protectedValue`, `payout`, `premium`, `premiumRateBps` (with `tierBps`/`durationBps`), `toTokenUnits`. Every formula above lives here — nothing inline elsewhere. Also exports `PRICE_DECIMALS = 8`, the single definition the registry's feed check is built on.
...
Events: `NoteCreated(noteId, owner, asset, amount, entryPrice, level, expiry, premium, protectedValue, liability)`, `NoteSettled(noteId, settlementPrice, payout, recipient)`, `Deposited depositor/amount`, `Withdrawn to/amount`, `CapacityReserved(noteId, payer, premium, liability)`, `CapacityReleased(noteId, liability)`, `PayoutExecuted(noteId, to, amount)`, `ProtocolFeeAccrued(noteId, amount)`, `ProtocolFeesClaimed(to, amount)`, `FeeTermsChanged(protocolFeeBps, treasury)`, `AssetRegistered(token, feed)`, `AssetStatusChanged(token, active)`, `AssetFeedUpdated(token, feed)`, `BufferChanged(bps)`, `NoteContractSet(noteContract)`, `OwnershipTransferred(prev, next)`, `SequencerUptimeFeedSet(feed)`, `SequencerGracePeriodSet(seconds)`.

Custom errors: `UnsupportedAsset`, `AssetInactive`, `StalePrice`, `StaleRound`, `InvalidPrice`, `InsufficientCapacity`, `InsufficientVaultBalance`, `InsufficientPosition`, `InsufficientShares`, `SharesTooSmall`, `AssetConcentration`, `InvalidExposureCap`, `NotExpired`, `AlreadySettled`, `InvalidLevel`, `InvalidDuration`, `InvalidAmount`, `InvalidDecimals`, `Unauthorized`, `InvalidOwner`, `TransferFailed`, `BufferTooHigh`, `BufferBreachesReserves`, `EncumberedFunds`, `PayoutExceedsLiability`, `NotNoteContract`, `AlreadySet`, `NoteNotFound`, `AlreadyRegistered`, `NotRegistered`, `ZeroAddress`, `StalenessTooHigh`, `UnsupportedFeedDecimals`, `UnsupportedTokenDecimals`, `FeeTooHigh`, `SequencerDown`, `SequencerGracePeriodNotOver`, `InvalidSequencerFeed`, `GracePeriodTooLong`.
...
1. `reserved ≤ totalDeposits − totalDeposits×buffer/10_000` at all times — including across backer `withdraw`/`redeem` (capped at `availableCapacity()`) and owner `setBufferBps` (a raise that would breach it is refused). No withdrawal path and no admin call can move this line; the fee sweep cannot either, because `pendingProtocolFees` never counted toward `totalDeposits`.
